...
> Quick toggle-able sizing overlay for Spot strategies (a.k.a. “Risk Overlay”, “Vol Sizer”).
...
และสามารถเปิด/ปิดโหมดนี้ได้ด้วย flag `RISK_MODE_ON` (หรือ `VOL_SIZER_ENABLED` ถ้าอยากตั้งชื่อใหม่ให้สื่อความในระบบของคุณ)

### TL;DR การใช้งานเร็ว

1) ตั้งค่า flag เปิด/ปิดโหมด (ดูหัวข้อถัดไป)  
2) ป้อนข้อมูลหลัก: สัญญาณ direction + base_size จากกลยุทธ์, ราคาแท่งล่าสุด, และ ATM IV ถ้ามี  
3) โหมดคำนวณ risk_score → แปลงเป็น weight multiplier → คูณกับ base_size เพื่อหาขนาดไม้สุดท้าย  
4) ถ้าไม่อยากใช้ ให้ปิด flag แล้วระบบจะกลับไปใช้ base_size ตรง ๆ

---

## 0. สถานะโหมด

| Flag หลัก | ความหมาย | ข้อสังเกต |
| --- | --- | --- |
| `RISK_MODE_ON` = `true` | เปิดโหมด “เบรก/คันเร่ง” ให้ช่วยปรับขนาดไม้ | ใช้ชื่ออื่นได้ เช่น `VOL_SIZER_ENABLED`, แต่ควรสื่อว่าเป็นตัวเปิด/ปิด overlay เท่านั้น |
| `RISK_MODE_ON` = `false` | ปิดโหมด → ส่งคำสั่งตามกลยุทธ์หลักตามเดิม (base_size เดิม) | ใช้สำหรับ backtest เทียบก่อน/หลัง หรือกรณีตลาด event พิเศษที่ไม่อยากลดน้ำหนักอัตโนมัติ |

> สำหรับ production ควรมีช่องทางเปิด/ปิด flag ได้ทั้งระดับ config file, env var และ runtime override (เช่นผ่าน CLI หรือ UI toggle) เพื่อให้คนเฝ้าเครื่องกด “ปิดโหมด” ได้ทันทีเวลาเกิดเหตุฉุกเฉิน

...
def run(cfg_path, out_dir):
    ...
    for pc in cfg.get("pairs", []):
        paircfg = PairConfig(
            ...
            fees_taker=float(pc["fees"]["taker"]) * fee_multiplier,
            fees_maker=float(pc["fees"]["maker"]) * fee_multiplier,
            buy_d=float(pc["buy_ladder"]["d_buy"]),
            buy_m=float(pc["buy_ladder"]["m_buy"]),
            buy_n=int(pc["buy_ladder"]["n_steps"]),
            buy_spacing=str(pc["buy_ladder"].get("spacing_mode", "geometric")),
            buy_multipliers=pc["buy_ladder"].get("d_multipliers"),
            buy_max_drop=float(pc["buy_ladder"].get("max_step_drop", 0.25)),
            buy_size_mode=str(pc["buy_ladder"].get("size_mode", "geometric")),
            buy_gap_mode=str(pc["buy_ladder"].get("gap_mode", "additive")),
            buy_gap_factor=float(pc["buy_ladder"].get("gap_factor", 1.0)),
            buy_base_order_quote=pc["buy_ladder"].get("base_order_quote"),
            buy_max_quote_per_leg=float(pc["buy_ladder"].get("max_quote_per_leg", 0.0)),
            buy_max_total_quote=float(pc["buy_ladder"].get("max_total_quote", 0.0)),

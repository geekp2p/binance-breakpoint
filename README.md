...
docker compose up --build live  # แนะนำใส่ --build ครั้งแรกเพื่อสร้าง image ก่อน
...
- หากราคาปัจจุบันต่ำกว่าราคาเป้าหมายของสัญญาณขายเกิน `general.max_exit_slippage_pct` (เช่น 0.3%) ระบบจะข้ามการส่งคำสั่ง EXIT รอบนั้นและบันทึกเหตุการณ์ `EXIT_ABORTED_SLIPPAGE`
...
- เปิด **stuck recovery guardrail** ที่ฝังในโค้ดหลัก (เหมาะกับอาการ ladder/micro ไม่ขยับหลายวัน) โดยเพิ่มใน `features` ของแต่ละคู่ เช่น
  ```yaml
  features:
    stuck_recovery:
      enabled: true
      stage1_bars: 4320   # ~3 วันบน 1m klines
      stage2_bars: 10080  # ~7 วันบน 1m klines
      # สามารถปรับ d_buy/micro bands ต่อสเตจ หรือปล่อยให้ใช้ค่าดีฟอลต์ในโค้ด
  ```
  เมื่อเปิดใช้ ระบบจะค่อย ๆ ขยาย d_buy/แคบแบนด์ micro ตามจำนวนแท่งที่ไม่มีการซื้อขาย และจะรีเซ็ตกลับอัตโนมัติเมื่อมีการเทรดใหม่
